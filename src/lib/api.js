...
async function call(action, payload = {}, timeoutMs = 20000) {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      return { success: false, error: '서버 응답이 지연되고 있어요. 잠시 후 다시 시도해주세요.' }
    }
    return { success: false, error: '서버에 연결하지 못했어요.' }
  } finally {
    clearTimeout(t)
  }
}
...
export function startKakaoLogin() {
  return new Promise((resolve, reject) => {
    ...
    // 타임아웃을 login() 전체에 걸면 PC에서 사용자가 ID/비번 입력하는 시간까지
    // 포함돼 억울하게 끊긴다(모바일은 카톡 앱이 즉시 처리해 안 걸림).
    // → 사용자 입력 단계엔 타임아웃을 두지 않고, 카카오 SDK 콜백에 맡긴다.
    // 서버 지연은 call() 내부의 fetch 타임아웃이 따로 처리한다.
    window.Kakao.Auth.login({
      success: () => {
        window.Kakao.API.request({
          ...
          fail: () => reject(new Error('카카오 사용자 정보를 가져오지 못했어요.'))
        })
      },
      fail: () => {
